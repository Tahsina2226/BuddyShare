...
export default function PaymentHistoryComponent() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      if (isAdmin && selectedUser) {
        url = `/admin/payments/user/${selectedUser}`;
      } else if (isHost) {
        url = `/host/payments`;
      }
      ...
      if (response.data.success) {
        ...
        const uniquePayments = removeDuplicatePayments(succeededPayments);
  ...
  const removeDuplicatePayments = (payments: any[]) => {
    ...
    payments.forEach((payment) => {
      const key = `${payment.event?._id || "no-event"}_${
        payment.user?._id || payment.user
      }_${payment.amount}_${payment.status}_${new Date(
        payment.createdAt
      ).getTime()}`;

      if (
        !uniqueMap.has(key) ||
        new Date(payment.createdAt) > new Date(uniqueMap.get(key).createdAt)
      ) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-green-500/10 to-emerald-600/10 backdrop-blur-sm mb-6 p-6 border-2 border-green-500/20 rounded-xl">
        ...
        <div className="mt-2 text-white/60 text-xs">
          Showing only successful/completed payments
        </div>
      </div>
      ...
      {showDebug && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-blue-500/10 p-6 border border-blue-500/30 rounded-xl"
        >
          <h4 className="flex items-center gap-2 mb-4 font-bold text-blue-400">
          ...
          <div className="space-y-2 text-blue-300/80 text-sm">
            ...
            <div>
              <strong>Total Successful Payments:</strong> {payments.length}
            </div>
            <div>
              <strong>Total Revenue:</strong>{" "}
              {formatCurrency(stats.totalRevenue)}
            </div>
            <div className="mt-4 pt-4 border-blue-500/30 border-t">
              <strong>Note:</strong> Only showing succeeded/completed payments
            </div>
          </div>
        </motion.div>
      )}
